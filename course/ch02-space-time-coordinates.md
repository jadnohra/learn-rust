...
<li><a href="#three-primitives">Three Primitives</a></li>
<li><a href="#bugs-as-interaction-failures">Bugs as Interaction Failures</a></li>
<li><a href="#paradigms-and-languages">Paradigms and Languages</a></li>
<li><a href="#features-as-interaction-solutions">Features as Interaction Solutions</a></li>
</ul>
</div>
...
Three primitives rooted in physics explain every category of memory bug. The framework is abstract. It applies to CPU caches, databases, distributed systems, and programming languages alike (drawing inspiration from the [Abstractor](/abstractor/)). Rust's solution only makes sense once the problem is clear.

This framework explains why Rust works the way it does.

<details>
<summary>Checkpoint</summary>
<p>You understand this chapter explains the fundamental problem. You know the framework is abstract and applies broadly. You are motivated to learn the foundation.</p>
</details>
...
<details>
<summary>Checkpoint</summary>
<p>You see coherence as a universal problem. Physics creates distance, distance forces copies, copies require coherence. You see the pattern from CPU caches to distributed systems.</p>
</details>
...
**COORDINATES.** How we refer to data. Names at compile-time, addresses at runtime, references, pointers. Coordinates tell you where space is. Throughout this course, "coord" and "address" mean the same thing.

State sits outside the three primitives. It is central to the coherence problem. State is data that changes over time. A value at an address that may differ on the next read. Stateless computation has no coherence problem because there is nothing to get out of sync. Most coherence problems involve state.

<details>
<summary>Checkpoint</summary>
<p>You know the three axes: SPACE, TIME, COORDINATES. You understand state is central but not a primitive. You have vocabulary to describe memory bugs.</p>
</details>
...
These are well-known bugs. Each maps to a specific interaction between primitives.

Each diagram shows a bug as a timeline. Space exists for a period. Coordinates point to it. The bug occurs when the relationship between primitives breaks down.
...
<details>
<summary>Checkpoint</summary>
<p>You see each bug as a breakdown between primitives. You understand the diagrams. You recognize these bugs from your own experience.</p>
</details>
...
**Clojure.** Data structures appear mutable. They are immutable. Modifying a map returns a new map that shares structure with the old one. Explicit constructs like atoms and refs handle state changes.
...
<details>
<summary>Checkpoint</summary>
<p>You see how different languages approach coherence. You understand the spectrum from "eliminate the problem" to "programmer responsibility."</p>
</details>
...
Languages solve coherence problems with specific features. Each feature targets a specific interaction between primitives. Many features appear in multiple languages. Some are ancient. Some are recent. All address the same underlying physics.
...
**Ownership / move semantics.** Formalized in C++11, central to Rust. Each value has exactly one owner. Passing a value transfers ownership. The original binding becomes invalid. Ownership eliminates aliasing by default. The programmer must copy explicitly.
...
**Nullable types / Option.** ML languages introduced this in the 1970s. References must be explicitly nullable. Code must handle the "no data here" case to compile. Nullable types eliminate null pointer dereferences. Used by ML, Haskell, Rust, Swift, Kotlin.
...
**Closures capturing environment.** Dates to Lisp in 1958. A closure that captures variables forces the language to decide what happens to them. Ownership and borrowing rules determine whether the closure copies, borrows, or moves the captured variables.
...
**SPACE x TIME x COORDINATES.** The full problem. The full interaction of all three primitives resists elegant solutions. Multiple threads accessing shared mutable state is fundamentally hard. The features in this category are trade-offs. Mutexes block. Atomics are limited. Channels require restructuring code. Immutability forbids mutation. Each gives up something to gain coherence.
...
**Atomics.** Hardware support dates to the 1970s. Hardware-level indivisible operations. Hardware guarantees that single reads and writes complete without interruption, enabling lock-free coordination. Limited to simple operations. Correct usage is difficult.

**Channels / message passing.** Dates to CSP in 1978. Threads send data to each other by transferring ownership. The sender loses access, the receiver gains it. Requires restructuring code around message flow.
...
**Linear types.** Theoretical foundation dates to the 1980s. Each value must be used exactly once. The compiler enforces that no code path leaks or double-uses a resource. Rust's ownership is a practical form of this.
...
<details>
<summary>Checkpoint</summary>
<p>You have a catalog of features organized by which primitive interaction they solve. You know the historical context. You understand the trade-offs. You have a framework for understanding memory bugs. You are ready to see how Rust applies these ideas.</p>
</details>
